...
import { normalizeCnpj } from '../../utils/normalizers'
  ...
  async function handleNextStep() {
    try {
      ...
      if (err.response.status === 400) {
        setError(
          'O CNPJ informado é inválido ou ainda não foi cadastrado no Sefaz.',
        )
      } else if (err.response.status === 401) {
        setError('O CNPJ informado precisa ser referente a uma MEI.')
      } else if (err.response.status === 429) {
        setError(
          'O sistema está sobrecarregado, por favor, aguarde um instante.',
        )
      } else if (err.response.status === 504) {
        setError(
          'O sistema está indisponível, por favor, tente novamente mais tarde.',
        )
      } else {
        setError(
          'Houve um erro inesperado, pedimos desculpas, por favor tente novamente mais tarde.',
        )
      }
    } finally {
      setLoading(false)
